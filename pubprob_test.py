...
num_generations = 50  # ジェネレーション数
mutation_rate = 0.01  # 突然変異確率
simulation = 1  # シミュレーション回数
...
class Agent:
    def __init__(self, id):
        ...
        self.reputation = np.full(num_agent, 'G', dtype='<U1')  # 評価配列（初期値は全てGood）
    ...
    for generation in tqdm(range(num_generations)):
        ...
        for period in range(num_periods):
            ...
            for donor_id, recipient_id in pairs:
                ...
                evaluator_ids = np.arange(num_agent)
                not_donor_mask = evaluator_ids != donor_id
                private_mask = np.random.rand(num_agent) > probability

                # 評価対象（私的評価をする者）
                valid_ids = evaluator_ids[not_donor_mask & private_mask]

                # recipient の評判を取り出す（各 evaluator 視点）
                recipient_reps = np.array([agents[i].reputation[recipient_id] for i in valid_ids])

                # 各 evaluator の評価（リスト内包表記）
                updated_reps = np.array([
                    agents[i].update_reputation(action, recipient_reps[j])
                    for j, i in enumerate(valid_ids)
                ])

                # 評価反転マスクを適用
                error_mask = np.random.rand(len(valid_ids)) < error_rate_evaluation
                updated_reps[error_mask] = np.where(updated_reps[error_mask] == 'G', 'B', 'G')

                # 評価を反映
                for i, evaluator_id in enumerate(valid_ids):
                    agents[evaluator_id].reputation[donor_id] = updated_reps[i]

                # 公的評価を適用（公的評価に従う者）
                public_mask = not_donor_mask & (~private_mask)
                for evaluator_id in evaluator_ids[public_mask]:
            ...
            # → NumPy配列に変換
            records_array = np.array(action_records, dtype=object)
            donors = records_array[:, 0].astype(int)
            recipients = records_array[:, 1].astype(int)
            actions = records_array[:, 2]

            # mask: 協力行動（action == 'C'）のみ抽出
            cooperation_mask = actions == 'C'

            # 対象となる donor / recipient ID を抽出
            cooperate_donors = donors[cooperation_mask]
            cooperate_recipients = recipients[cooperation_mask]

            # payoff 配列（更新前）
            payoffs = np.array([agent.payoff for agent in agents])

            # 更新を適用
            np.add.at(payoffs, cooperate_donors, -cost_of_cooperation)
            np.add.at(payoffs, cooperate_recipients, benefit_of_cooperation)

            # 反映
            for i, agent in enumerate(agents):
                agent.payoff = payoffs[i]
...
def main():
    ...
    error_rates_action = 0.001
    error_rates_evaluation = 0.001
    benefit_values = 5
